...
PlayerControl3* PlayerControl3::create(GameObject* owner) {
	auto pAI = new PlayerControl3();
	pAI->init(owner);
	return pAI;
}

bool PlayerControl3::init(GameObject* owner) {
	if (!Ability::init(owner)) {
		return false;
	}
	this->setName("PlayerControl3");
...
void PlayerControl3::update() {
	// input event
	if (AEInputCheckCurr('S')) {
		onKeyDown(DOWN);
	}
	else if (AEInputCheckCurr('W')) {
		onKeyDown(UP);
	}
...
void PlayerControl3::onKeyDown(int direction) {
...
bool PlayerControl3::isOppositeDirection(int direction1, int direction2) {
	switch (direction1) {
	case UP:
	{
...
void PlayerControl3::getCollisionPoint(int direction, float &x, float &y) {
...
void PlayerControl3::getCollisionPoint(int direction, float &x1, float &y1, float &x2, float &y2) {
	float curX = this->getOwner()->getPositionX();
	float curY = this->getOwner()->getPositionY();
	float delta = 1.5f;
	switch (direction) {
...
void PlayerControl3::eatBeans() {
	...
	if (beans == WINNUM) {
		SystemController::getInstance()->setNextLevel(LevelP1win::create());
	}
}