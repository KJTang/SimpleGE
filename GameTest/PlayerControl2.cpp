...
PlayerControl2* PlayerControl2::create(GameObject* owner) {
	auto pAI = new PlayerControl2();
	pAI->init(owner);
	return pAI;
}

bool PlayerControl2::init(GameObject* owner) {
	if (!Ability::init(owner)) {
		return false;
	}
	this->setName("PlayerControl2");
...
void PlayerControl2::onKeyDown(int direction) {
...
bool PlayerControl2::isOppositeDirection(int direction1, int direction2) {
	switch (direction1) {
	case UP:
	{
...
void PlayerControl2::getCollisionPoint(int direction, float &x, float &y) {
...
void PlayerControl2::getCollisionPoint(int direction, float &x1, float &y1, float &x2, float &y2) {
	float curX = this->getOwner()->getPositionX();
	float curY = this->getOwner()->getPositionY();
	float delta = 1.5f;
	switch (direction) {
...
void PlayerControl2::eatBeans() {
	...
	if (beans == WINNUM) {
		SystemController::getInstance()->setNextLevel(LevelP2win::create());
	}
}